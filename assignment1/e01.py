...
x = tf.random_uniform([])
y = tf.random_uniform([])
z = tf.cond(x > y, lambda: tf.add(x,y), lambda: tf.subtract(x,y))
...
y = tf.gather(x, tf.where(tf.greater(x,30)))
...
###############################################################################
x = tf.random_uniform(dtype=tf.float32, shape=(6,6))
det = tf.matrix_determinant(x)

x,det = sess.run([x,det])
print(x,det)
print("\n")
...
out = tf.cond(tf.reduce_mean(x-y) < 0, f1, f2)
